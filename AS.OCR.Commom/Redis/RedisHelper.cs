...
namespace AS.OCR.Commom.Util
{
    public class PooledRedisClientHelper : IDisposable
    {
        ...
        /// <summary>
        /// 如果Key不存在则设置hash型key某个字段的值 原子性
        /// </summary>
        /// <param name="key"></param>
        /// <param name="field"></param>
        /// <param name="value"></param>
        public static void SetEntryInHashIfNotExists(string key, string field, string value)
        {
            using (IRedisClient redis = PooleClient.GetClient())
            {
                redis.SetEntryInHashIfNotExists(key, field, value);
            }
        }
    ...
    public class ConfigHelper
    {
        ...
        public static string ReadWriteHosts
        {
            get
            {
                var RedisAddress = ConfigurationUtil.RedisAddress;
                if (string.IsNullOrWhiteSpace(RedisAddress))
                    throw new Exception("RedisAddress 配置项为空");
                return RedisAddress;
            }
        }
